...
export function performCombo(
    ...
    const enemies = updated.filter(
        (u) =>
            ...
            Math.abs(u.position[0] - finalPos[0]) +
                Math.abs(u.position[1] - finalPos[1]) <=
                range
    );
    if (enemies.length) {
        // pick weakest enemy
...
// Basic melee attack (range = 1)
export function basicAttack(target: Unit, damage = 20): Unit {
    return { ...target, hp: Math.max(0, target.hp - damage) };
}

// Ranged attack (range >1)
export function rangedAttack(target: Unit, damage = 15): Unit {
    return { ...target, hp: Math.max(0, target.hp - damage) };
}

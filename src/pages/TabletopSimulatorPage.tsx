  ...
  const handleCloseOutcome = () => {
    if (activeNodeId) {
      setCampaignNodes(prev =>
        prev.map(n => (n.id === activeNodeId ? { ...n, completed: true, unlocked: true } : n))
      );
    }
    setScenario(null);
    setBrief(null);
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-6 py-10">
      <header className="mb-6">
        <h1 className="text-3xl font-bold text-zinc-100">AI-Powered Tabletop Simulator</h1>
        <p className="mt-2 text-zinc-300">
          Warhammer-inspired skirmishes with AI-driven battles. Choose a mission, play the scenario, and view the
      ...
      <div
        id="game-container"
        className="flex h-[72vh] flex-col overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 shadow-sm sm:h-[78vh] md:h-[80vh] md:flex-row"
      >
        {/* Sidebar */}
        <aside className="order-2 max-h-[40vh] w-full min-w-0 overflow-y-auto border-t border-zinc-800 bg-zinc-950/40 p-4 md:order-1 md:h-full md:w-80 md:border-t-0 md:border-r">
          <GameDashboard
            gameState={gameState}
            ...
            completeNode={id =>
              setCampaignNodes(prev => prev.map(n => (n.id === id ? { ...n, completed: true } : n)))
            }
        ...
        {/* Main */}
        <section className="relative order-1 flex-1 overflow-auto p-4 md:order-2">
          {scenario ? (
            <>
            ...
            <div className="flex h-full items-center justify-center">
              <p className="text-zinc-400">Select a mission from the sidebar to begin.</p>
            </div>
          )}
        </section>
      </div>
